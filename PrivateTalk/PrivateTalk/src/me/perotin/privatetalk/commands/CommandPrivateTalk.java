...
public class CommandPrivateTalk implements CommandExecutor {
	...
	public boolean onCommand(CommandSender s, Command arg1, String arg2, String[] args) {
		...
		if (args[0].equalsIgnoreCase("accept")) {
			if (invites.containsKey(p)) {
				Conversation convo = invites.get(p);
				convo.add(p);
				invites.remove(p);
				p.sendMessage(ChatColor.YELLOW + "You have joined " + ChatColor.GREEN + convo.getName());
		...
		if (args[0].equalsIgnoreCase("promote")) {
			if (args.length != 2) {
				...
				if (!c.getOwner().getName().equalsIgnoreCase(p.getName()) && !p.hasPermission("pt.promote")) {
					...
					if(!c.playerInConversation(t)){
						p.sendMessage(ChatColor.RED + t.getName() + " must be in your conversation before you promote that player!");
						return true;
					}
					c.setOwner(t);
		...
		if (args[0].equalsIgnoreCase("create")) {
			...
			if (args.length == 2) {
				...
				p.sendMessage(ChatColor.WHITE + "Conversation " + ChatColor.GREEN + convoName + ChatColor.WHITE + " - " + ChatColor.GREEN + "OWNER (" + ChatColor.WHITE + p.getName() + ChatColor.GREEN + ") PUBLIC (" + ChatColor.WHITE + "false" + ChatColor.GREEN + ")");
				...
				if (toToggle.getOwner().getName().equalsIgnoreCase(p.getName())) {
					...
					if (args.length == 1) {
						if (pub) {
							...
							p.sendMessage(ChatColor.YELLOW + "You have set " + ChatColor.GREEN + toToggle.getName() + ChatColor.YELLOW + " to " + ChatColor.RED + "private!");
							return true;
						} else {
							toToggle.setPublic(true);
							p.sendMessage(ChatColor.YELLOW + "You have set " + ChatColor.GREEN + toToggle.getName() + ChatColor.YELLOW + " to " + ChatColor.GREEN + "public!");
							return true;
						}
					} else if (args.length == 2) {
						String bol = args[1];
			...
			if (Conversation.getConversation(p) == null) {
				...
				if (Conversation.getConversation(p).getOwner().getName() != p.getName() && !p.isOp()) {
					...
					if (c.playerInConversation(toInvite)) {
						...
						p.sendMessage(ChatColor.YELLOW + "Invited " + ChatColor.GREEN + toInvite.getName() + ChatColor.YELLOW + " to " + ChatColor.GREEN + c.getName());
						toInvite.sendMessage(ChatColor.YELLOW + "You have been invited to the conversation " + ChatColor.GREEN + c.getName());
						toInvite.sendMessage(ChatColor.YELLOW + "To join, type " + ChatColor.GREEN + "/pt accept " + ChatColor.YELLOW + "or " + ChatColor.RED + "/pt deny");
						return true;
					}
				}
			}
		...
		} else if (!args[0].equalsIgnoreCase("create") && !args[0].equalsIgnoreCase("list") && !args[0].equalsIgnoreCase("leave") && !args[0].equalsIgnoreCase("kick") && !args[0].equalsIgnoreCase("info") && !args[0].equalsIgnoreCase("accept") && !args[0].equalsIgnoreCase("deny") && !args[0].equalsIgnoreCase("invite") && !args[0].equalsIgnoreCase("public") && !args[0].equalsIgnoreCase("join") && !args[0].equalsIgnoreCase("toggle") && !args[0].equalsIgnoreCase("delete")
